...
def login():
    ...
    if form.validate_on_submit():  # 如果提交
        ...
        session[
            "username"] = user.username  # 将usser_name写入session, 后面用户判断用户是否登录
...
def goods_list(supercat_id=None):  # supercat_id 为商品大分类ID
    ...
    page_data = Goods.query.filter_by(supercat_id=supercat_id).paginate(
        page=page, per_page=12)
    hot_goods = Goods.query.filter_by(supercat_id=supercat_id).order_by(
        Goods.views_count.desc()).limit(7).all()
    return render_template('home/goods_list.html',
                           page_data=page_data,
                           hot_goods=hot_goods,
...
def goods_detail(id=None):  # id 为商品ID
    ...
    db.session.commit()  # 提交数据
    # 获取左侧热门商品
    hot_goods = Goods.query.filter_by(subcat_id=goods.subcat_id).order_by(
        Goods.views_count.desc()).limit(5).all()
    # 获取底部相关商品
    similar_goods = Goods.query.filter_by(subcat_id=goods.subcat_id).order_by(
        Goods.addtime.desc()).limit(5).all()
    return render_template('home/goods_detail.html',
                           goods=goods,
                           hot_goods=hot_goods,
...
def goods_search():
    ...
    if keywords:
        # 使用like实现模糊查询
        page_data = Goods.query.filter(
            Goods.name.like("%" + keywords + "%")).order_by(
                Goods.addtime.desc()).paginate(page=page, per_page=12)
    else:
        page_data = Goods.query.order_by(Goods.addtime.desc()).paginate(
            page=page, per_page=12)
    hot_goods = Goods.query.order_by(Goods.views_count.desc()).limit(7).all()
    return render_template("home/goods_search.html",
                           page_data=page_data,
                           keywords=keywords,
                           hot_goods=hot_goods)
...
def shopping_cart():
    ...
    cart = Cart.query.filter_by(user_id=int(user_id)).order_by(
        Cart.addtime.desc()).all()
    if cart:
...
def order_list():
    ...
    """
    user_id = session.get('user_id', 0)
    orders = OrdersDetail.query.join(Orders).filter(
        Orders.user_id == user_id).order_by(Orders.addtime.desc()).all()
...
def collect_add():
    ...
    collect = collect_add.query.filter_by(  # 根据用户ID和景区ID判断是否该收藏
        user_id=int(user_id), scenic_id=int(scenic_id)).count()
    # 已收藏
    if collect == 1:
        data = dict(ok=0)  # 写入字典
    # 未收藏进行收藏
    if collect == 0:
        collect = Collect(user_id=int(user_id), scenic_id=int(scenic_id))
...
def collect_list():
    ...
    page_data = Collect.query.filter_by(user_id=session['user_id']).order_by(
        Collect.addtime.desc()).paginate(page=page, per_page=3)  # 使用分页方法
    return render_template('home/collect_list.html',
                           page_data=page_data)  # 渲染模板
