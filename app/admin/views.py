...
def goods_edit(id=None):
    ...
    form.supercat_id.choices = [
        (v.id, v.cat_name) for v in SuperCat.query.all()
    ]  # 为super_cat_id添加属性
...
def login():
    ...
    if form.validate_on_submit():  # 验证提交表单
        ...
        admin = Admin.query.filter_by(
            manager=data["manager"]).first()  # 查找Admin表数据
...
def user_list():
    ...
    if keyword:
        ...
        page_data = User.query.filter(filters).order_by(
            User.addtime.desc()).paginate(page=page, per_page=5)
    else:
...
def supercat_add():
    ...
    if request.method == 'POST':
        ...
        data = SuperCat(cat_name=cat_name, )
        db.session.add(data)
        db.session.commit()
...
def supercat_del():
    ...
    if request.method == 'POST':
        ...
        db.session.query(SuperCat).filter(
            SuperCat.id.in_(cat_ids)).delete(synchronize_session=False)
        db.session.commit()
        return redirect(url_for("admin.supercat_list"))
...
def subcat_del():
    ...
    if request.method == 'POST':
        ...
        # 使用in_ 方式批量删除，需要设置synchronize_session为False,而 in 操作估计还不支持
        # 解决办法就是删除时不进行同步，然后再让 session 里的所有实体都过期
        db.session.query(SubCat).filter(
            SubCat.id.in_(cat_ids)).delete(synchronize_session=False)
        db.session.commit()
        return redirect(url_for("admin.subcat_list"))
...
def orders_list():
    ...
    if keywords:
        page_data = Orders.query.filter_by(id=keywords).order_by(
            Orders.addtime.desc()).paginate(page=page, per_page=10)
    else:
        page_data = Orders.query.order_by(Orders.addtime.desc()).paginate(
            page=page, per_page=10)
...
def orders_detail():
    ...
    orders = OrdersDetail.query.join(Orders).filter(
        OrdersDetail.order_id == order_id).all()
...
def topgoods():
    ...
    销量排行榜(前10位)
    """
    orders = OrdersDetail.query.order_by(
        OrdersDetail.number.desc()).limit(10).all()
